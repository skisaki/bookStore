...
def getProducts():
    return [
        {
            'id': 0,
            'title': '아마존 웹 서비스 인 액션 (Amazon Web Service In Action)',
            'descriptionHtml': '<a href="http://m.yes24.com/goods/detail/39011465">아마존 웹 서비스 인 액션 (Amazon Web Service In Action)</a><br> AWS 입문자를 위한 시원시원한 실습서<br> 이 책은 AWS를 클라우드 인프라의 핵심인 컴퓨팅, 저장소, 네트워킹을 중심으로 소개한다. 구체적으로는 클라우드 컴퓨팅 개론과 계정 생성 같은 기초, AWS API로 AWS의 모든 기능을 프로그램적으로 제어하는 인프라 자동화, 데이터를 안전하게 보관하는 다양한 기법, 사설 네트워크로 시스템을 격리하는 방법, 고가용성과 장애허용 설계 기법 등을 설명한다.'
        }
    ]
